...
image_testing_app <- function() {
  create_testing_app(
    ...
    function(input, output, session) {
      ...
      output$pacifico <- renderImage(do_image(family = "Pacifico", height = height, width = width, ratio))
      output$default <- renderImage(do_image(family = "", height = height, width = width, ratio))
    }
  )
}
...
plot_testing_app <- function() {
  create_testing_app(
    ...
    function(input, output) {
      ...
      output$pacifico <- renderPlot(do_plot(), bg = info1$bg)
      output$default <- renderPlot(do_plot(family = ""), bg = info1$bg)
    }
  )
}
