...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("body{background-color:#444; color:#e4e4e4}")),
    tags$style(HTML("a{color:#e39777}"))
  ),
  titlePanel("Plot auto-theming"),
  tabsetPanel(
    tabPanel(
      "ggplot2",
      selectInput(
        "ggplot2_example", "Choose an example",
        choices = names(ggplot2_examples), selected = "GeomSmooth"
      ),
    ...
    tabPanel(
      "lattice",
      selectInput(
        "lattice_example", "Choose an example",
        choices = names(lattice_examples)
      ),
      plotOutput("lattice_plot")
    ),
    tabPanel(
      "base",
      selectInput(
        "base_example", "Choose an example", choices = names(base_examples)
      ),
      plotOutput("base_plot")
    ),
    tabPanel(
      "info",
      HTML("Computed style accessible via <code>shiny::getCurrentOutputInfo()</code>"),
      tagAppendAttributes(verbatimTextOutput("info"), class = "shiny-report-theme")
    ),
    tabPanel(
      "reactive auto-theme",
      selectInput(
...
server <- function(input, output, session) {
  ...
  output$ggplot_plot <- renderCachedPlot({
    ggplot2_examples[[input$ggplot2_example]]
  }, cacheKeyExpr = { list(input$ggplot2_example) })

  output$lattice_plot <- renderCachedPlot({
    eval(lattice_examples[[input$lattice_example]])
  }, cacheKeyExpr = { list(input$lattice_example)  })

  output$base_plot <- renderCachedPlot({
    eval(base_examples[[input$base_example]])
  }, cacheKeyExpr = { list(input$base_example) })

  output$info <- renderPrint({
    str(getCurrentOutputInfo())
  })

  output$reactive_accent <- renderPlot({
    ggplot(diamonds[sample(nrow(diamonds), 1000), ], aes(carat, price)) +
      geom_point(alpha = 0.2) +
      geom_smooth() +
      facet_wrap(~cut) + ggtitle("Diamond price by carat and cut")
  }, autoTheme = list(accent = reactive(input$accent_color)))
